...
set mysock(proc-$mysock(uno-chan)) "uno_control_pub"
set mysock(proc-$mysock(uno-nick)) "uno_control_priv"
nodouble $mysock(gamelist)
set mysock(users-$mysock(uno-chan)) ""

proc uno_control_pub { nick text } {
  global mysock
  fsend $mysock(sock) ":$mysock(uno-nick) PRIVMSG $mysock(uno-chan) :\002PUB \002 $nick > $text"
}

proc uno_control_priv { nick text } {
  global mysock
  fsend $mysock(sock) ":$mysock(uno-nick) PRIVMSG $mysock(uno-chan) :\002PRIV\002 $nick > $text"
}
